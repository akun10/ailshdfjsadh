...
byte pass[] = { 10, 4, 20, 2, 0,2 };
byte fail[] = { 20, 4, 10, 2, 0,2};
byte mute[] = {20,4,16,4,12,4};
byte soundOn[] = {12,4,16,4,20,4};
void playMelody(byte *mel, int len) {
  for (int i = 0; i < len; i++)
    beep(mel[i*2], 1000 / mel[i*2 + 1]);
}
